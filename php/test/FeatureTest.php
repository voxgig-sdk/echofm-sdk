...
class FtCtrl extends EchofmControl
{
...
class FtClient
{
    ...
    private EchofmUtility $utility;

    public function __construct(EchofmUtility $utility, array $options)
    {
    ...
    public function get_utility(): EchofmUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (EchofmContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new EchofmUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (EchofmContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = EchofmConfig::make_feature($name);
    ...
    private static function buildUrl(EchofmSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof EchofmError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new EchofmResponse($fetched);
        }
    ...
    private function fail(EchofmContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (EchofmContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
