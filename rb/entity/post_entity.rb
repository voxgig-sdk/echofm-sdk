...
class PostEntity
  ...
  def data_set(args)
    if args
      @_data = EchofmHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = EchofmHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Post, Hash] the loaded Post; raises EchofmError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = EchofmHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
