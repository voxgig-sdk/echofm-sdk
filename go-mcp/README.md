...
[MCP](https://modelcontextprotocol.io) server exposing the Echofm SDK as
two agent tools — `echofm_list` and `echofm_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/echofm-mcp)
make build

# 2. Provide credentials via the environment
export ECHOFM_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user echofm \
  -- /absolute/path/to/echofm-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./echofm-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// echofm_list: first page of records
{ "entity": "post" }
{ "entity": "post", "query": { } }

// echofm_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/echofm-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user echofm \
     -- "$PWD"/dist/*/echofm-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `echofm_list` and `echofm_load` tools now appear
   in new sessions. Ask the agent to *"list post using echofm"*
   and it calls `echofm_list` with `{"entity":"post"}`.
...
```sh
export ECHOFM_APIKEY=sk_live_xxx            # API key
export ECHOFM_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `echofm_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `echofm_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `ECHOFM_APIKEY` | API key sent with every request. |
| `ECHOFM_BASE` | Optional override of the API base URL. |
...
```sh
./echofm-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"echofm_load","arguments":{"entity":"post","query":{"id":1}}}}'
```
