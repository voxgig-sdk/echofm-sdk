// Command Echofm-mcp is an MCP server exposing the Echofm
// SDK as MCP tools.
//
// Two tools are registered:
//
//	Echofm_list  — list records of an entity
//	Echofm_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: ECHOFM_APIKEY carries the API key and
	// ECHOFM_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("ECHOFM_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("ECHOFM_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewEchofmSDK(opts)
