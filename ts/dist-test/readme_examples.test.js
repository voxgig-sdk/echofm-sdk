...
function isRunnable(code) {
    return (/new\s+EchofmSDK\b/.test(code) ||
        /\bEchofmSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+EchofmSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/EchofmSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
