...
(0, node_test_1.describe)('PostDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-post', async (t) => {
        ...
        const query = {};
        if (setup.live) {
            params.post_id = 469191;
        }
        else {
            params.post_id = 'direct01';
        }
        const result = await client.direct({
            path: 'api/views/{post_id}',
            method: 'GET',
            params,
            query,
        });
...
function liveScenariosActive() { return false && process.env.ECHOFM_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.EchofmSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['ECHOFM_TEST_POST_ENTID'];
    ...
    const client = new __1.EchofmSDK({
        base: 'http://localhost:8080',
