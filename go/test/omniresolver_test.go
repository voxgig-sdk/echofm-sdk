...
import (
	...
	omni "github.com/voxgig-sdk/echofm-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.EchofmError); is {
				out := map[string]any{
					"name":    "EchofmError",
...
func omniCtx(arg any, client *sdk.EchofmSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.EchofmSDK); is {
			client = live
