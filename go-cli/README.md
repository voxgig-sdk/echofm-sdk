...
boru-driven command-line client **and** interactive REPL for the Echofm
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/echofm-cli)
make build
...
./echofm-cli load 1 post            # {id:1} shorthand
./echofm-cli load '{id:1}' post       # explicit match map

# 5. Override the API base URL for a single call
ECHOFM_BASE=https://api.example.com ./echofm-cli load 1 post
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/echofm-cli
   ```
...
```sh
./echofm-cli load 1 post          # scalar shorthand for {id:1}
./echofm-cli load '{id:1}' post     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `echofm>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/echofm-cli`. |
...
(`./echofm-cli <expr>`) and interactively (the REPL), and expressions compose the
