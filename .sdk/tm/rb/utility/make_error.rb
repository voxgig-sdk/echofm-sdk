...
module EchofmUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = EchofmContext.new({}, nil)
    end
    op = ctx.op || EchofmOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || EchofmResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(EchofmError) ? err.msg : err.to_s
    ...
    sdk_err = EchofmError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(EchofmError)
