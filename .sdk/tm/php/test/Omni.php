...
//    only - a typed EchofmContext there would make every ctx
...
final class EchofmOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new EchofmContext($clean, null);
    ...
    public static function err_from_map($m): ?EchofmError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new EchofmError(is_string($code) ? $code : '', $msg);
    }
