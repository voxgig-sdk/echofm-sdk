...
// Echofm SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class EchofmMakeError
{
    public static function call(?EchofmContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new EchofmContext([], null);
        }
        $op = $ctx->op ?? new EchofmOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new EchofmResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof EchofmError) ? $err->msg : (string)$err;
        $msg = "EchofmSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new EchofmError('', $msg, $ctx);
